classdef singleElevator < handle
   ...
   methods
       ...
       function upFunction(process,dt)
           if ~isempty(process.upRequest) % up request is not empty
               ...
               if dist == 0
                   ...
                   process.elevatorProcessor.openOutsideDoor(process.currentFloor,processor.NO,1);
                   process.door = 2;
                   process.t = dt;
                   process.upRequest(1) = [];
               ...
               elseif dist == 1
                   ...
                   if process.v == process.speed && process.a ~= 0
                       process.a = 0;
           ...
           elseif ~isempty(process.downRequest) && process.downRequest(1)*process.floorHeight >= process.h
       ...
       function downFunction(process,dt)
           if ~isempty(process.downRequest) % down request is not empty
               ...
               if dist == 0
                   process.a = 0;
                   process.ElevatorUI.openInsideDoor();
                   process.elevatorProcessor.openOutsideDoor(process.currentFloor,processor.NO);
                   process.door = 2;
                   process.t = dt;
                   process.downRequest(1) = [];
               elseif dist>0 && dist<1 
           ...
           else % elevator's down request is empty
               % total up requests
               united = flip(unique([process.temp, process.elevatorProcessor.tempDown, process.downRequest]));
               if ~isempty(united)
                   process.temp = [];
       ...
       function stayFunction(process,dt)
           ...
           tdown = flip(unique([process.downRequest, processor.elevatorProcessor.tempDown]));
